...
def get_campaign_kategories(db: Session = Depends(get_db)):
    return db.query(CampaignCategories).all()

@router.get("/{id}", response_model=CampaignCategoriesRead)
def get_campaign_kategorie(id: int, db: Session = Depends(get_db)):
    ck = db.query(CampaignCategories).filter(CampaignCategories.id == id).first()
    if not ck:
        raise HTTPException(status_code=404, detail="CampaignCategories not found")
    return ck

@router.post("/", response_model=CampaignCategoriesRead)
def create_campaign_kategorie(ck: CampaignCategoriesCreate, db: Session = Depends(get_db)):
    db_ck = CampaignCategories(**ck.dict())
    db.add(db_ck)
    db.commit()
    db.refresh(db_ck)
    return db_ck
